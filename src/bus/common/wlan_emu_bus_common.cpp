...
#define COMPARE_INT_RANGE(min_value, max_value, set_value) do { \
    if ((set_value) < (min_value)) { \
        ...
        wlan_emu_print(wlan_emu_log_level_err, "Value %d is greater than the maximum value (%d)\n", (set_value), (max_value)); \
        return RETURN_ERR; \
    } \
} while (0)
...
static elem_node_map_t *insert_table_row(elem_node_map_t *table_root, char *node_name)
{
    ...
    while(next_node != NULL)
    {
        ...
        {
            current_node = next_node;
            wlan_emu_print(wlan_emu_log_level_dbg,"%s:%d child name=[%s], Token = [%s] already present\n",__func__, __LINE__, next_node->name, node_name);
...
static int check_dm_min_max_data_range(long int min_data, long int max_data, raw_data_t *bus_set_data)
{
...
static void duplicate_node_ref(elem_node_map_t* child_table_node, elem_node_map_t* parent_node)
{
    ...
    if ((parent_node->node_elem_data_len != 0) && (child_table_node->node_elem_data == NULL)) {
        ...
        wlan_emu_print(wlan_emu_log_level_dbg,"%s:%d Added node new data info for %s--data len:%d\n", __func__,
