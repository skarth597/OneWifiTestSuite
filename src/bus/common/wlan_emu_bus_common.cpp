...
#define COMPARE_INT_RANGE(min_value, max_value, set_value)                                      \
    do {                                                                                        \
        if ((set_value) < (min_value)) {                                                        \
            wlan_emu_print(wlan_emu_log_level_err,                                              \
                "Value %d is less than the minimum value (%d)\n", (set_value), (min_value));    \
            return RETURN_ERR;                                                                  \
        } else if ((set_value) > (max_value)) {                                                 \
            wlan_emu_print(wlan_emu_log_level_err,                                              \
                "Value %d is greater than the maximum value (%d)\n", (set_value), (max_value)); \
            return RETURN_ERR;                                                                  \
        }                                                                                       \
    } while (0)
...
{
    wlan_emu_bus_t *bus = get_emu_bus_obj();
    return &bus->bus_cb_mux.bus_mux_mutex;
}

elem_node_map_t *get_bus_mux_sub_cb_map(void)
{
    wlan_emu_bus_t *bus = get_emu_bus_obj();
    return bus->bus_cb_mux.bus_sub_cb_root;
}

elem_node_map_t *get_bus_mux_reg_cb_map(void)
{
    wlan_emu_bus_t *bus = get_emu_bus_obj();
    return bus->bus_cb_mux.bus_reg_cb_root;
}
...
static void node_elems_free(elem_node_map_t *node, node_traversal_cb_param_t param)
{
    ...
    if (node->node_data_type == node_elem_reg_data) {
        ...
        if (reg_node_data != NULL) {
            data_model_properties_t *data_model_prop = &reg_node_data->data_model_prop;
            if (data_model_prop->str_validation) {
                for (uint32_t index = 0; index < data_model_prop->num_of_str_validation; index++) {
                    free(data_model_prop->str_validation[index]);
                }
                free(data_model_prop->str_validation);
            }
        }
    }
    ...
    wlan_emu_print(wlan_emu_log_level_info, "node delete [%p]:%s:%s\n", node, node->name,
        node->full_name);
    free(node);
}

static void node_elem_recurse_traversal(elem_node_map_t *node,
    node_elem_traversal_arg_t *input_action)
{
    VERIFY_NULL(node);
    ...
    if (node->reference_childs != reference_child_node) {
        ...
        wlan_emu_print(wlan_emu_log_level_info, "mirror link node found [%p]->child:%p\n", node,
            node->child);
    }
...
static void node_elem_traversal(elem_node_map_t *node, node_elem_traversal_arg_t *input_action)
{
    ...
    if (node->reference_childs != reference_child_node) {
        ...
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d mirror link node found [%p]->child:%p\n",
            __func__, __LINE__, node, node->child);
    }
...
static bus_error_t free_node_elems(elem_node_map_t *node)
{
...
static char const *get_type_string(bus_element_type_t type)
{
    switch (type) {
...
static void print_elem(elem_node_map_t *node, int level)
{
    wlan_emu_print(wlan_emu_log_level_dbg,
        "%s:%d %*s[name:%s type:%s full_name: %s addr:%p, parent:%p, child:%p]\n", __func__,
        __LINE__, level * 2, level ? " " : "", node->name, get_type_string(node->type),
        node->full_name, node, node->parent, node->child);

    bus_mux_reg_node_data_t *reg_node_data = (bus_mux_reg_node_data_t *)node->node_elem_data;
    if ((node->node_data_type == node_elem_reg_data) && (reg_node_data != NULL)) {
        bus_callback_table_t *p_cb_table = &reg_node_data->cb_table;
        wlan_emu_print(wlan_emu_log_level_dbg,
            "%s:%d [full_name: %s get_cb:%p set_cb:%p add_cb:%p remove:%p event:%p method:%p]\n",
            __func__, __LINE__, node->full_name, p_cb_table->get_handler, p_cb_table->set_handler,
            p_cb_table->table_add_row_handler, p_cb_table->table_remove_row_handler,
            p_cb_table->event_sub_handler, p_cb_table->method_handler);
    } else {
        wlan_emu_print(wlan_emu_log_level_dbg,
            "%s:%d [cb:%d not found full_name: %s cb_pointer:%p]\n", __func__, __LINE__,
...
elem_node_map_t *retrieve_instance_elem_node(elem_node_map_t *root, const char *elmentName)
{
    ...
    BUS_MUX_LOCK(get_bus_mux_mutex());
    strncpy(name, elmentName, sizeof(name) - 1);
    ...
    while (token != NULL) {
        ...
        if (strcmp(next_node->name, token) == 0) {
            wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d token_found!\n", __func__, __LINE__);
            token_found = 1;
            ...
            while (next_node != NULL) {
                if (strcmp(next_node->name, token) == 0) {
                    wlan_emu_print(wlan_emu_log_level_info, "token_found!\n");
                    token_found = 1;
                    current_node = next_node;
                    next_node = current_node->child;
                    break;
        ...
        if (token && next_node && next_node->parent &&
            next_node->parent->type == bus_element_type_table) {
            if (!is_wild_card && !strcmp(token, "*")) {
                is_wild_card = true;
            }
...
static elem_node_map_t *link_tables_with_node(elem_node_map_t *parent_node)
{
    ...
    while (next_node) {
        if (next_node->child == NULL && is_string_digit(next_node->name)) {
            ...
            wlan_emu_print(wlan_emu_log_level_info, "%s:%d Added duplicate node ref for=%s\n",
                __func__, __LINE__, next_node->full_name);
        }
...
elem_node_map_t *get_bus_node_info(elem_node_map_t *cb_root, char *name)
{
    ...
    if (cb_root == NULL || name == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: user cb root map not found\n", __func__,
            __LINE__);
        return NULL;
    }

    strncpy(recv_name, name, sizeof(recv_name) - 1);

    node_elem = retrieve_instance_elem_node(cb_root, recv_name);
    if (node_elem != NULL) {
        return node_elem;
    }
    wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d  bus callback not found=%s\n", __func__, __LINE__,
        recv_name);
...
{
    wlan_emu_bus_t *bus_obj = get_emu_bus_obj();
...
static elem_node_map_t *insert_table_row(elem_node_map_t *table_root, char *node_name)
{
    ...
    while (next_node != NULL) {
        wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d child name=[%s], Token = [%s]\n", __func__,
            __LINE__, next_node->name, node_name);
        if (strcmp(next_node->name, node_name) == 0) {
            current_node = next_node;
            wlan_emu_print(wlan_emu_log_level_dbg,
                "%s:%d child name=[%s], Token = [%s] already present\n", __func__, __LINE__,
            ...
            next_node = current_node->nextSibling;
            if (next_node == NULL) {
                wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d Create Sibling [%s]\n", __func__,
                    __LINE__, node_name);
                ...
                wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d Full name [%s]\n", __func__, __LINE__,
                    buff);
                strncpy(temp_node->full_name, buff, sizeof(temp_node->full_name) - 1);
                strncpy(temp_node->name, node_name, sizeof(temp_node->name) - 1);
...
static bus_error_t bus_add_table_row(elem_node_map_t *table_root, unsigned int num_of_table_row)
{
...
elem_node_map_t *bus_insert_elem_node(elem_node_map_t *root, bus_mux_data_elem_t *elem)
{
    ...
    if (current_node == NULL || elem == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "Root node or Element is NULL!!\n");
        return NULL;
    }
    ...
    strncpy(name, elem->full_name, sizeof(name) - 1);
        ...
        if (next_node == NULL) {
            if (create_child) {
                ...
                if (current_node == root) {
                    strncpy(temp_node->full_name, token, sizeof(temp_node->full_name) - 1);
                } else {
                    snprintf(buff, sizeof(buff), "%s.%s", current_node->full_name, token);
                    strncpy(temp_node->full_name, buff, sizeof(temp_node->full_name) - 1);
                }
                strncpy(temp_node->name, token, sizeof(temp_node->name) - 1);
    ...
    if (ret == 0) {
        ...
        if (elem->type == bus_element_type_table) {
            ...
            strncpy(rowTemplate->name, "{i}", sizeof(rowTemplate->name) - 1);
            snprintf(buff, sizeof(buff), "%s.%s", current_node->full_name, rowTemplate->name);
            strncpy(rowTemplate->full_name, buff, sizeof(rowTemplate->full_name) - 1);
...
static int check_dm_min_max_data_range(long int min_data, long int max_data,
    raw_data_t *bus_set_data)
{
    if (min_data == 0 && max_data == 0) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: int range validation is not needed\n",
            __func__, __LINE__);
        return RETURN_OK;
    }

    wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d: set data type:%d\n", __func__, __LINE__,
...
static void duplicate_node_ref(elem_node_map_t *child_table_node, elem_node_map_t *parent_node)
{
    ...
    if ((parent_node->node_elem_data_len != 0) && (child_table_node->node_elem_data == NULL)) {
        ...
        memcpy(child_table_node->node_elem_data, parent_node->node_elem_data,
            parent_node->node_elem_data_len);
        child_table_node->node_elem_data_len = parent_node->node_elem_data_len;
        wlan_emu_print(wlan_emu_log_level_dbg,
            "%s:%d Added node new data info for %s--data len:%d\n", __func__, __LINE__,
            child_table_node->full_name, parent_node->node_elem_data_len);
    }
    ...
    if (child_table_node->child == NULL) {
        ...
        wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d Added duplicate node ref for %s\n", __func__,
...
bus_error_t bus_table_add_row(elem_node_map_t *p_root_node, char *p_name_space,
    uint32_t table_index)
{
    ...
    if (retrieve_table_elem != NULL) {
        ...
        if (table_node_elem == NULL) {
            wlan_emu_print(wlan_emu_log_level_err, "%s:%d table node is not found:%s.%d\n",
                __func__, __LINE__, p_name_space, table_index);
            return bus_error_general;
        }
        wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d Added table node:%s.%d\n", __func__, __LINE__,
            p_name_space, table_index);
...
static int validate_dm_string_param(uint32_t num_of_str, char **str, char *set_str)
{
    if (num_of_str == 0) {
        wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d: string validation is not set\n", __func__,
            __LINE__);
    } else if (str == NULL || set_str == NULL) {
        wlan_emu_print(wlan_emu_log_level_dbg, "%s:%d: input string is NULL:%p:%p\n", __func__,
            __LINE__, str, set_str);
    } else {
        uint32_t index;
        ...
        if (str_found == false) {
            wlan_emu_print(wlan_emu_log_level_err, "%s:%d: string:%s validation is failed:%d\n",
                __func__, __LINE__, set_str, num_of_str);
            return RETURN_ERR;
        }
    }
...
int validate_dm_set_parameters(data_model_properties_t *data_model_prop, raw_data_t *bus_set_data)
{
    ...
    if (data_model_prop->data_permission == false) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: data is not permitted to set\n", __func__,
            __LINE__);
        return ret;
    }
    ...
    if (ret != RETURN_OK) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: set data min/max data range is not proper\n",
            __func__, __LINE__);
        return ret;
