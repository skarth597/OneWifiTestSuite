...
{
    return &g_bus_obj->desc;
}

static rbusError_t convert_bus_to_rbus_error_code(bus_error_t bus_error)
{
    ...
    default:
        rbus_error = RBUS_ERROR_BUS_ERROR;
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d unsupported bus error code:%02x\r\n",
...
static bus_data_type_t convert_rbus_to_bus_data_type(rbusValueType_t rbus_data_type)
{
    ...
    default:
        bus_data_type = bus_data_type_none;
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d unsupported bus data type:%02x\r\n", __func__,
            __LINE__, rbus_data_type);
        break;
    }
...
static rbusElementType_t convert_bus_to_rbus_elem_type(bus_element_type_t bus_elem_type)
{
    ...
    default:
        rbus_elem_type = RBUS_ELEMENT_TYPE_PROPERTY;
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d unsupported bus element type:%d\r\n",
...
static bus_error_t get_rbus_property_data(char *event_name, rbusProperty_t property, raw_data_t *bus_data)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s Rbus property=%s\n", __FUNCTION__, event_name);
    bus_data->data_type = convert_rbus_to_bus_data_type(type);
    switch (type) {
    case RBUS_STRING:
        bus_data->raw_data.bytes = (void *)rbusValue_GetString(value,
            (int *)&bus_data->raw_data_len);
        if (bus_data->raw_data.bytes == NULL) {
            wlan_emu_print(wlan_emu_log_level_err, "%s Rbus get string failed len=%d\n",
    ...
    case RBUS_BOOLEAN:
        ...
        break;
    case RBUS_BYTES:
        bus_data->raw_data.bytes = (void *)rbusValue_GetBytes(value,
            (int *)&bus_data->raw_data_len);
        if (bus_data->raw_data.bytes == NULL) {
            wlan_emu_print(wlan_emu_log_level_err, "%s Rbus get bytes is failed len=%d\n",
                __FUNCTION__, bus_data->raw_data_len);
        }
        break;
    default:
...
static bus_error_t convert_rbus_to_bus_error_code(rbusError_t rbus_error)
{
    ...
    default:
        bus_error = bus_error_general;
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d unsupported rbus error code:%02x\r\n",
...
static void *get_bus_cb_data_info(elem_node_map_t *cb_root, char *name)
{
    elem_node_map_t *mux_elem = get_bus_node_info(cb_root, name);
    if (mux_elem != NULL) {
        return mux_elem->node_elem_data;
    }
    wlan_emu_print(wlan_emu_log_level_err, "%s:%d Rbus callback info not found=%s\n", __func__,
        __LINE__, name);
    return NULL;
}

static rbusError_t rbus_table_add_row_handler(rbusHandle_t handle, char const *tableName,
    char const *aliasName, uint32_t *instNum)
{
    ...
    bus_mux_reg_node_data_t *reg_node_data = (bus_mux_reg_node_data_t *)get_bus_cb_data_info(
        get_bus_mux_reg_cb_map(), (char *)tableName);
    if (reg_node_data == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d rbus event name:%s, user cb not found\n",
...
static bool remove_substring(char *str, const char *sub)
{
    ...
    if (len > strlen(".{i}.")) {
        ...
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d: bus: removeSubstring is matched with %s\n",
...
static bus_error_t set_rbus_property_data(char *event_name, rbusProperty_t property, raw_data_t *bus_data)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d Rbus property:%s data type=%x set\r\n", __func__,
    ...
    case bus_data_type_object:
        wlan_emu_print(wlan_emu_log_level_err, "%s Rbus:%s value type not supported =%d\n",
            __func__, event_name, bus_data->data_type);
        break;
    default:
        wlan_emu_print(wlan_emu_log_level_err, "%s Rbus:%s value type not found =%d\n", __func__,
            event_name, bus_data->data_type);
        ret = bus_error_invalid_input;
        break;
    }
...
static void free_raw_data_struct(raw_data_t *p_data)
{
    if ((p_data->data_type == bus_data_type_string || p_data->data_type == bus_data_type_bytes) &&
        p_data->raw_data.bytes != NULL) {
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d free raw obj data type:%02x:%p\r\n",
            __func__, __LINE__, p_data->data_type, p_data->raw_data.bytes);
    ...
    rbusSetHandlerOptions_t *options)
{
    ...
    if (event_name == NULL) {
        ...
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d rbus data set end comp:%s\n", __func__,
            __LINE__, options->requestingComponent);
    }
        ...
        if (ret == bus_error_success) {
            ...
            if (ret_status != RETURN_OK) {
                wlan_emu_print(wlan_emu_log_level_err,
                    "%s:%d rbus event:%s, invalid data:%x operation\n", __func__, __LINE__,
                    event_name, bus_data.data_type);
...
static rbusError_t rbus_get_handler(rbusHandle_t handle, rbusProperty_t property,
    rbusGetHandlerOptions_t *options)
{
    ...
    if (event_name == NULL) {
        ...
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d rbus data get end comp:%s\n", __func__,
            __LINE__, options->requestingComponent);
    }
    ...
    bus_callback_table_t *user_cb = &reg_node_data->cb_table;
    if ((bus_data.data_type == bus_data_type_none) &&
        (reg_node_data->data_model_prop.data_format != 0)) {
    ...
    if (user_cb->get_handler != NULL) {
        ...
        if (ret == bus_error_success) {
            ...
            wlan_emu_print(wlan_emu_log_level_err, "%s:%d user cb processing failed:%d for %s\n",
...
static bus_error_t bus_data_get(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    if (rc != RBUS_ERROR_SUCCESS) {
        wlan_emu_print(wlan_emu_log_level_err,
            "%s:%d bus: rbus_get failed for [%s] with error [%d]\n", __func__, __LINE__, name, rc);
    ...
    case RBUS_BYTES:
        ...
        break;
    default:
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: bus: Invalid data_type:%d for name:%s.\n",
    ...
    if ((ptr) && (name) &&
        ...
        if (data->raw_data.bytes == NULL) {
            wlan_emu_print(wlan_emu_log_level_err,
                "%s:%d: bus: memory alloc is failed:%d for name:%s\n", __func__, __LINE__, len,
                name);
...
static bus_error_t bus_open(bus_handle_t *handle, char *component_name)
{
    ...
    if (rc != RBUS_ERROR_SUCCESS) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: bus: rbus_open() failed:%d, component:%s\n",
            __func__, __LINE__, rc, component_name);
        return convert_rbus_to_bus_error_code(rc);
    }
    wlan_emu_print(wlan_emu_log_level_info,
        "%s:%d: bus: rbus_open() is successful for component:%s, \
       rc:%d, handle:%p\n",
...
static bus_error_t bus_close(bus_handle_t *handle)
{
    ...
    if (rc != RBUS_ERROR_SUCCESS) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: bus: rbus_close() failed:%d\n", __func__,
            __LINE__, rc);
        return convert_rbus_to_bus_error_code(rc);
    }
    wlan_emu_print(wlan_emu_log_level_info,
        "%s:%d: bus: rbus_close() is successful. rc:%d, handle:%p\n", __func__, __LINE__, rc,
        handle->u.rbus_handle);
    return convert_rbus_to_bus_error_code(rc);
}
    ...
    rbusEventSubscription_t *subscription)
{
    ...
    bus_mux_sub_node_data_t *sub_node_data = (bus_mux_sub_node_data_t *)get_bus_cb_data_info(
        get_bus_mux_sub_cb_map(), event_name);
...
static void rbus_sub_ex_async_handler(rbusHandle_t handle, rbusEventSubscription_t *subscription,
    rbusError_t error)
{
    (void)handle;
    bus_error_t bus_error = convert_rbus_to_bus_error_code(error);

    wlan_emu_print(wlan_emu_log_level_info, "%s:%d rbus sub ex async cb triggered\n", __func__,
        __LINE__);
    if (subscription) {
        char *event_name = (char *)subscription->eventName;
        void *userData = subscription->userData;
...
static void bus_sub_cb_registration(char *event_name, rbus_sub_callback_table_t *cb_table,
    bus_sub_callback_table_t *user_cb)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d sub_user_cb_set:%d event_name:%s\n", __func__,
        __LINE__, user_cb_set, event_name);
    ...
    if (user_cb_set == true) {
        ...
        if (bus_insert_elem_node(sub_cb_mux_map, &node_elem) == NULL) {
            wlan_emu_print(wlan_emu_log_level_err, "%s:%d user_cb_set failure for event_name:%s\n",
                __func__, __LINE__, event_name);
            return;
        }
    } else {
        wlan_emu_print(wlan_emu_log_level_info,
            "%s:%d user_sub_cb_set:%d not needed for event:%s\n", __func__, __LINE__, user_cb_set,
            event_name);
    }
}

static bus_error_t bus_event_subscribe_ex(bus_handle_t *handle, bus_event_sub_t *l_sub_info_map,
    int num_sub, int timeout)
{
    ...
    if (p_rbus_handle == NULL || l_sub_info_map == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "%s bus: Input param is NULL\n", __func__);
        return bus_error_invalid_input;
    }

    sub_info_map = (rbusEventSubscription_t *)calloc(1, num_sub * sizeof(rbusEventSubscription_t));
    if (sub_info_map == NULL) {
        wlan_emu_print(wlan_emu_log_level_err,
            "%s:%d bus: bus_event_subscribe_ex() calloc is failed\n", __func__, __LINE__);
        return bus_error_out_of_resources;
    }

    for (int index = 0; index < num_sub; index++) {
        user_cb.sub_handler = reinterpret_cast<bus_event_sub_handler_t>(
            l_sub_info_map[index].handler);
        ...
        sub_info_map[index].filter = reinterpret_cast<rbusFilter_t>(l_sub_info_map[index].filter);
        sub_info_map[index].interval = l_sub_info_map[index].interval;
        sub_info_map[index].duration = l_sub_info_map[index].duration;
        sub_info_map[index].handler = reinterpret_cast<void *>(rbus_cb.sub_handler);
...
static bus_error_t bus_set(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    if (rc != RBUS_ERROR_SUCCESS) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d bus: rbus_set() failed:%d for name:%s\n",
...
static bus_event_sub_action_t convert_rbus_to_bus_sub_action_type(rbusEventSubAction_t rbus_sub_action)
{
    ...
    default:
        bus_sub_action = bus_event_action_unsubscribe;
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d unsupported bus sub action:%02x\r\n",
...
static rbusError_t rbus_event_sub_handler(rbusHandle_t handle, rbusEventSubAction_t action,
    char const *eventName, rbusFilter_t filter, int32_t interval, bool *autoPublish)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d rbus cb triggered for %s\n", __func__, __LINE__,
        eventName);
    bus_mux_reg_node_data_t *reg_node_data = (bus_mux_reg_node_data_t *)get_bus_cb_data_info(
        get_bus_mux_reg_cb_map(), (char *)eventName);
    if (reg_node_data == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d rbus event name:%s, user cb not found\n",
...
static rbusError_t rbus_table_remove_row_handler(rbusHandle_t handle, char const *rowName)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d rbus cb triggered for %s\n", __func__, __LINE__,
        rowName);
    bus_mux_reg_node_data_t *reg_node_data = (bus_mux_reg_node_data_t *)get_bus_cb_data_info(
        get_bus_mux_reg_cb_map(), (char *)rowName);
    if (reg_node_data == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d rbus event name:%s, user cb not found\n",
    ...
    rbusObject_t outParams, rbusMethodAsyncHandle_t asyncHandle)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d rbus cb triggered for %s\n", __func__, __LINE__,
        methodName);
    bus_mux_reg_node_data_t *reg_node_data = (bus_mux_reg_node_data_t *)get_bus_cb_data_info(
        get_bus_mux_reg_cb_map(), (char *)methodName);
    if (reg_node_data == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d rbus event name:%s, user cb not found\n",
    ...
    if (user_cb->method_handler != NULL) {
        ...
        if (ret == bus_error_success) {
            ret = user_cb->method_handler((char *)methodName, &bus_input_data, &bus_output_data,
                (void *)asyncHandle);
            if (ret != bus_error_success) {
                wlan_emu_print(wlan_emu_log_level_err,
                    "%s:%d user cb processing failed:%d for %s\n", __func__, __LINE__, ret,
...
static bool map_bus_user_cb_with_rbus(bus_data_element_t *data_element,
    rbusCallbackTable_t *cb_table)
{
    bool user_cb_set = false;
    ...
    if (user_cb->method_handler != NULL) {
        cb_table->methodHandler = reinterpret_cast<decltype(cb_table->methodHandler)>(
            rbus_method_handler);
        user_cb_set = true;
    }

    wlan_emu_print(wlan_emu_log_level_info, "%s:%d user_cb_set:%d event_name:%s\n", __func__,
...
static void mux_bus_cb_registration(bus_data_element_t *data_element, bool user_cb_set)
{
    ...
    strncpy(event_name, data_element->full_name, sizeof(event_name) - 1);
    ...
    if (user_cb_set == true) {
        ...
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d user_cb_set:%d not needed for event:%s\n",
            __func__, __LINE__, user_cb_set, event_name);
    }
}

static bus_error_t bus_reg_data_elements(bus_handle_t *handle, bus_data_element_t *data_element,
    uint32_t num_of_element)
{
    ...
    wlan_emu_print(wlan_emu_log_level_err, "%s:%d bus: bus_reg_data_elements() hdl:%p, \
        num_of_element:%d\n",
    ...
    rbus_dataElements = (rbusDataElement_t *)calloc(num_of_element, 1 * sizeof(rbusDataElement_t));
    if (rbus_dataElements == NULL) {
        wlan_emu_print(wlan_emu_log_level_err,
            "%s:%d bus: bus_reg_data_elements() calloc is failed\n", __func__, __LINE__);
        return bus_error_out_of_resources;
    }
    ...
    for (index = 0; index < num_of_element; index++) {
        ...
        user_cb_set = map_bus_user_cb_with_rbus(&data_element[index],
            &rbus_dataElements[index].cbTable);

        rc = rbus_regDataElements(p_rbus_handle, 1, &rbus_dataElements[index]);
        if (rc != RBUS_ERROR_SUCCESS) {
            wlan_emu_print(wlan_emu_log_level_err, "%s:%d: bus: rbus_regDataElements failed. \
                rc:%d for %s\n",
        ...
        if (data_element[index].type == bus_element_type_table) {
            ...
            if ((data_element[index].num_of_table_row) && (remove_substring(name, sub) == 1)) {
                for (table_index = 1; table_index <= data_element[index].num_of_table_row;
                    ...
                    if (rc != RBUS_ERROR_SUCCESS) {
                        wlan_emu_print(wlan_emu_log_level_err,
                            "%s() bus: rbusTable_addRow failed:%d for %s\n", __func__, rc, name);
                    }
                }
            }
        }
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d: bus: rbus elem reg success. rc:%d\n", __func__,
        __LINE__, rc);
    ...
    unsigned int size)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d: bus: bus_raw_event_publish() is successful \
        name:%s, rc:%d\n",
        __func__, __LINE__, name, rc);
    return convert_rbus_to_bus_error_code(rc);
}
    ...
    char const *param_str)
{
    ...
    if (rc != RBUS_ERROR_SUCCESS) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: bus: rbus_setStr() failed for param:%s, \
          rc:%d\n",
            __func__, __LINE__, param_name, rc);
    }
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d: bus: bus_set_string() is successful for \
          param:%s, rc:%d\n",
        __func__, __LINE__, param_name, rc);
    return convert_rbus_to_bus_error_code(rc);
}

static bus_error_t bus_event_subscribe(bus_handle_t *handle, char const *event_name, void *cb,
    void *userData, int timeout)
{
    ...
    user_cb.sub_handler = reinterpret_cast<bus_event_sub_handler_t>(cb);
    user_cb.sub_ex_async_handler = NULL;
...
static bus_error_t bus_event_publish(bus_handle_t *handle, char const *name, raw_data_t *data)
{
    ...
    if ((rc != RBUS_ERROR_SUCCESS) && (rc != RBUS_ERROR_NOSUBSCRIBERS)) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: bus: rbusEvent_Publish Event failed:%d, \
            name:%s\n",
            __func__, __LINE__, rc, name);
    } else {
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d: bus: bus_event_publish() is successful. \
            name:%s, rc:%d\n",
            __func__, __LINE__, name, rc);
    }
    ...
    int num_sub, void *l_sub_handler, int timeout)
{
    ...
    for (int index = 0; index < num_sub; index++) {
        user_cb.sub_handler = reinterpret_cast<bus_event_sub_handler_t>(
            l_sub_info_map[index].handler);
        user_cb.sub_ex_async_handler = reinterpret_cast<bus_event_sub_ex_async_handler_t>(
            l_sub_handler);
        bus_sub_cb_registration((char *)l_sub_info_map[index].event_name, &rbus_cb, &user_cb);
        sub_info_map[index].eventName = l_sub_info_map[index].event_name;
        sub_info_map[index].filter = reinterpret_cast<rbusFilter_t>(l_sub_info_map[index].filter);
    ...
    ret = rbusEvent_SubscribeExAsync(p_rbus_handle, sub_info_map, num_sub,
        rbus_cb.sub_ex_async_handler, timeout);
    ...
    raw_data_t *input_data, raw_data_t *output_data, uint8_t input_bus_data)
{
    ...
    if (input_bus_data == BUS_METHOD_SET) {
        if (input_data->data_type == bus_data_type_string) {
            if (false ==
                ...
                wlan_emu_print(wlan_emu_log_level_err,
                    "%s: bus: Invalid value '%s' for the parameter %s\n\r", __func__,
                    input_data->raw_data.bytes, paramName);
            }
        } else {
            wlan_emu_print(wlan_emu_log_level_err,
                "%s: bus: Invalid data_type '%d' for the parameter %s\n\r", __func__,
                input_data->data_type, paramName);
        }
    }

    rbusProperty_Init(&prop, (char const *)paramName, value);
    ...
    if ((RBUS_ERROR_SUCCESS == rc) && (input_bus_data == BUS_METHOD_GET)) {
        ...
        case bus_data_type_string:
            ...
            wlan_emu_print(wlan_emu_log_level_info,
                "%s bus: bus_method_invoke-string: string:%s, len=%d\n", __func__,
                output_data->raw_data.bytes, output_data->raw_data_len);
            break;
        case bus_data_type_uint32:
        ...
        case bus_data_type_bytes:
            ...
            output_data->raw_data_len = (unsigned int)len;
            wlan_emu_print(wlan_emu_log_level_info, "%s bus: bus_method_invoke-bytes: len=%d\n",
                __func__, output_data->raw_data_len);
            break;
        default:
            wlan_emu_print(wlan_emu_log_level_err, "%s bus:value type not found =0x%x\n", __func__,
                output_data->data_type);
            rc = RBUS_ERROR_INVALID_INPUT;
            break;
        }
    } else {
        wlan_emu_print(wlan_emu_log_level_err, " %s failed for  with err: '%s'\n\r", __func__,
...
static bus_error_t bus_event_unsubscribe(bus_handle_t *handle, char const *event_name)
{
    ...
    if (rc != RBUS_ERROR_SUCCESS) {
        wlan_emu_print(wlan_emu_log_level_err,
            "%s:%d bus: rbusEvent_Unsubscribe failed for"
            " [%s] with error [%d]\n",
...
static bus_error_t bus_unreg_data_elements(bus_handle_t *handle, uint32_t num_of_element,
    bus_data_element_t *data_element)
{
    ...
    wlan_emu_print(wlan_emu_log_level_info, "%s:%d bus: bus_unreg_data_elements() hdl:%p, \
        num_of_element:%d\n",
        __func__, __LINE__, p_rbus_handle, num_of_element);
    if (p_rbus_handle == NULL || data_element == NULL) {
        wlan_emu_print(wlan_emu_log_level_err, "%s bus: input param is NULL\n", __func__);
    ...
    rbus_dataElements = (rbusDataElement_t *)calloc(1, num_of_element * sizeof(rbusDataElement_t));
    if (rbus_dataElements == NULL) {
        wlan_emu_print(wlan_emu_log_level_err,
            "%s:%d bus: bus_unreg_data_elements() calloc is failed\n", __func__, __LINE__);
        return bus_error_out_of_resources;
    }
    ...
    rc = rbus_unregDataElements(p_rbus_handle, num_of_element, rbus_dataElements);
    if (rc != RBUS_ERROR_SUCCESS) {
        wlan_emu_print(wlan_emu_log_level_err, "%s:%d: bus: rbus_unregDataElements failed. \
            rc:%d.\n",
            __func__, __LINE__, rc);
        free(rbus_dataElements);
        return convert_rbus_to_bus_error_code(rc);
    ...
    for (index = 0; index < num_of_element; index++) {
        ...
        wlan_emu_print(wlan_emu_log_level_info, "%s:%d %s entry removed\n", __func__, __LINE__,
